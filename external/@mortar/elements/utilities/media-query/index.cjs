...
var MteMediaQuery = class extends MteContentBase {
  ...
  generateQueryStyles(smallerThan, largerThan, query4, styles9) {
    ...
    const smt = import_styles8.config.grid.breakpoint[smallerThan] ?? mapNumberToPx(smallerThan);
    const lgt = import_styles8.config.grid.breakpoint[largerThan] ?? mapNumberToPx(largerThan);
    let queryStyles = {};
  ...
  updateInstanceStyles(changedProps) {
    ...
    if (changedProps.has("queryStyles")) {
      this.setInstanceStyle(
        "media-query",
        (0, import_styles8.selectorFactory)(`${this.instanceStyleSelectorRoot}:host${this.selectorSuffix}`),
        this.queryStyles ?? {}
      );
    }
  }
