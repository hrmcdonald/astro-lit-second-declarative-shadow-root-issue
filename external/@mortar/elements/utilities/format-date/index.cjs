...
// src/utilities/format-date/index.ts
var format_date_exports = {};
__export(format_date_exports, {
  MteFormatDate: () => MteFormatDate
});
...
var MteFormatDate = class extends LangMixin(MteElement) {
  ...
  renderDate() {
    if (!this.date) {
      ...
      return import_lit14.html`<time datetime="${date.toISOString()}">${MteLocalizeService.formatDate(date, {
        lang: this.lang,
  ...
  render() {
    return import_lit14.html`${this.instanceStyles}${this.renderDate()}`;
  }
};
MteFormatDate.styles = [styles7];
__decorateClass([
