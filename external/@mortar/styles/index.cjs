...
__export(src_exports, {
  ...
  config: () => config,
  densityVariantsMixin: () => densityVariantsMixin,
  disabled: () => disabled,
  elevation: () => elevation,
  fillspacing: () => fillspacing,
  focusedMixin: () => focusedMixin,
  fontBaseMixin: () => fontBaseMixin,
  fontOverflowEllipsisMixin: () => fontOverflowEllipsisMixin,
  fontSetInheritMixin: () => fontSetInheritMixin,
  fontSetMixin: () => fontSetMixin,
  forEachPGVariant: () => forEachPGVariant,
  isHostClass: () => isHostClass,
  paletteGroupOptions: () => paletteGroupOptions,
  selector: () => selector,
  selectorFactory: () => selectorFactory,
  setConfig: () => setConfig,
  sizeModifier: () => sizeModifier,
  sizeVariantsMixin: () => sizeVariantsMixin,
  stateToken: () => stateToken,
  stateTokenMixin: () => stateTokenMixin,
  stripUnit: () => stripUnit,
  stripUnitFloat: () => stripUnitFloat,
  stripUnitInt: () => stripUnitInt,
  token: () => token,
...
// src/generated/maps/core-vars-getter-map.ts
...
// src/generated/maps/theme-vars-getter-map.ts
...
// src/generated/maps/core-vars-setter-map.ts
...
// src/generated/maps/theme-vars-setter-map.ts
...
// src/generated/maps/px-light-webgl-map.ts
...
// src/generated/maps/px-dark-webgl-map.ts
...
// src/config.ts
var config = __spreadValues(__spreadValues({}, core_vars_getter_map_default), theme_vars_getter_map_default);
var setConfig = __spreadValues(__spreadValues({}, core_vars_setter_map_default), theme_vars_setter_map_default);
var webGlConfig = {
  px: {
    light: __spreadValues({}, px_light_webgl_map_default),
    dark: __spreadValues({}, px_dark_webgl_map_default)
  }
};
...
var selectorFactory = (selectorRoot, {
  ...
  if (isHostClass(selectorRoot)) {
    if (internalInfix || infix) {
      return `${selectorRoot}(${internalInfix != null ? internalInfix : ""}${infix != null ? infix : ""})${internalPostfix != null ? internalPostfix : ""}${postfix != null ? postfix : ""}`;
    } else {
      return `${selectorRoot}${internalPostfix != null ? internalPostfix : ""}${postfix != null ? postfix : ""}`;
    }
  } else {
    return `${selectorRoot}${(_a = internalNonHostInfix != null ? internalNonHostInfix : internalInfix) != null ? _a : ""}${(_b = nonHostInfix != null ? nonHostInfix : infix) != null ? _b : ""}${(_c = internalNonHostPostfix != null ? internalNonHostPostfix : internalPostfix) != null ? _c : ""}${(_d = nonHostPostfix != null ? nonHostPostfix : postfix) != null ? _d : ""}`;
  }
};
var selector = (selectorRoot, { infix = "", postfix = "", nonHostInfix = null, nonHostPostfix = null } = {}) => {
  var _a, _b;
  if (isHostClass(selectorRoot)) {
    return infix ? `${selectorRoot}(${infix != null ? infix : ""})${postfix != null ? postfix : ""}` : `${selectorRoot}${postfix != null ? postfix : ""}`;
  } else {
    return `${selectorRoot}${(_a = nonHostInfix != null ? nonHostInfix : infix) != null ? _a : ""}${(_b = nonHostPostfix != null ? nonHostPostfix : postfix) != null ? _b : ""}`;
  }
};
var sizeModifier = (value, size) => {
  return `calc(${value} * ${config.size.scale[size]});`;
};
var stripUnit = (value) => {
  return value.replace(/{|px|pc|pt|in|mm|cm|ex|em|rem|vw|vh|vmin|vmax|ch|lh|}/, "");
};
var stripUnitInt = (value) => {
  return parseInt(value.replace(/{|px|pc|pt|in|mm|cm|ex|em|rem|vw|vh|vmin|vmax|ch|lh|}/, ""), 10);
};
var stripUnitFloat = (value) => {
  return parseFloat(value.replace(/{|px|pc|pt|in|mm|cm|ex|em|rem|vw|vh|vmin|vmax|ch|lh|}/, ""));
};
var sizeVariantsMixin = (selectorRoot, content) => {
  return Object.keys(config.size.scale).reduce((acc, variant) => {
    return `${acc} ${content(
      selectorFactory(selectorRoot, {
        infix: `[size="${variant}"]`,
        nonHostInfix: `.size--${variant}`
      }),
      variant
    )}`;
  }, "");
};
var densityVariantsMixin = (selectorRoot, content) => {
  return config.size.densities.reduce((acc, variant) => {
    return `${acc} ${content(
      selectorFactory(selectorRoot, {
        infix: `[density="${variant}"]`,
        nonHostInfix: `.density--${variant}`
      }),
      variant
    )}`;
  }, "");
...
var getOpacity = (opacity) => {
  var _a;
  return opacity = (_a = config.opacity[`${opacity}`]) != null ? _a : opacity;
};
var color = ({
  g: group = "core",
  p: palette,
...
var colorContrast = ({
  ...
  opacity = getOpacity(opacity);
  if (opacity) {
    return `rgba(var(--mte-${dashCase(group)}-${dashCase(palette)}-${color2}-c-rgb), ${opacity})`;
  } else {
    return `rgb(var(--mte-${dashCase(group)}-${dashCase(palette)}-${color2}-c-rgb))`;
  }
};
...
var token = ({ n: name, o: opacity = null }) => {
  opacity = getOpacity(opacity);
  if (opacity) {
    return `rgba(var(--mte-${dashCase(name)}-rgb), ${opacity})`;
  } else {
    return `rgb(var(--mte-${dashCase(name)}-rgb))`;
  }
};
var stateToken = ({ n: name, s: state, o: opacity }) => {
  opacity = getOpacity(opacity);
  if (opacity) {
    return `rgba(var(--mte-it-${dashCase(name)}-${state}-rgb), ${opacity})`;
  } else {
    return `rgba(var(--mte-it-${dashCase(name)}-${state}-rgb), var(--mte-it-${dashCase(
      name
    )}-${state}-a))`;
  }
};
var stateTokenMixin = (selectorRoot, {
  selectorOptions = { infix: "", postfix: "" },
  n: name,
  forStates = ["default", "hover", "active"],
  forProps = ["background"],
  useImportant = false,
  asOverlay = false
}) => {
  if (asOverlay) {
    forStates = forStates.filter((state) => state !== "default");
  }
  const selector2 = selectorFactory(selectorRoot, selectorOptions);
  const setInteractiveProps = ({ name: name2, state, forProps: forProps2, useImportant: useImportant2 = false }) => {
    return forProps2.reduce((acc, prop) => {
      return `${acc}
        ${prop}: ${stateToken({ n: name2, s: state })}${useImportant2 ? " !important;" : ";"}
      `;
    }, "");
  };
  return forStates.reduce((acc, state) => {
    return `${acc}
      ${selector2({ infix: state !== "default" ? `:${state}` : "" })}${asOverlay ? `::before {
          content: "";
          position: absolute;
          top: 0;
          right: 0;
          ...
          border-radius: inherit;` : ` {
      `}
        ${setInteractiveProps({ name, state, forProps, useImportant })}
      }
    `;
  }, "");
};
var focusedMixin = (selectorRoot, {
  selectorOptions = { infix: "", postfix: "" },
  selectorState = ":focus-visible",
  selectorOutlet = "::before",
  flush = false,
  radius = "4px",
  inset,
  radiusTopLeft,
  ...
  return `${selector(selectorRoot, __spreadProps(__spreadValues({}, selectorOptions), {
    infix: `${(_a = selectorOptions.infix) != null ? _a : ""}${selectorState != null ? selectorState : ""}:not(.disabled):not([disabled]):not(:disabled)`,
    postfix: `${(_b = selectorOptions.postfix) != null ? _b : ""}${selectorOutlet != null ? selectorOutlet : ""}`
  }))} {
    display: block;
    position: absolute;
    ${inset ? "" : `top: ${offsetTop};
      ...
      left: ${offsetLeft};`}
    ${inset ? `inset: ${inset};` : ""}
    border: solid 2px ${token({ n: "focus" })};
    box-shadow: 0 0 0 1px var(--mte-light);
...
// src/functions/typography.ts
var fontBaseMixin = (family = "sans") => `
  font-family: ${config.font.family[family]};
...
var fontSetMixin = ({
  ...
  const typeStyle = config.font.set[set];
  if (ratio) {
    return `
      ${fontBaseMixin(fontFamily)}
      font-size: calc(${typeStyle.fontSize} * ${ratio})${important ? " !important" : ""};
      font-weight: ${typeStyle.fontWeight}${important ? " !important" : ""};
      line-height: ${typeStyle.lineHeight}${important ? " !important" : ""};
    `;
  } else {
    return `
      ${fontBaseMixin(fontFamily)}
      font-size: ${typeStyle.fontSize}${important ? " !important" : ""};
      font-weight: ${typeStyle.fontWeight}${important ? " !important" : ""};
      line-height: ${typeStyle.lineHeight}${important ? " !important" : ""};
    `;
  }
};
var fontOverflowEllipsisMixin = () => `
  text-overflow: ellipsis;
  white-spacing: nowrap;
  overflow: hidden;
`;
var fontSetInheritMixin = () => `
  font-family: inherit;
  font-size: inherit;
  font-weight: inherit;
  line-height: inherit;
...
// src/functions/animation.ts
var transition = ({
  prop,
  duration = config.animation.transitionDuration.default,
  delay = "0ms",
  timingFn = config.animation.timingFunction.standard
}) => {
  return `${prop} ${duration} ${delay} ${timingFn}`;
};

// src/functions/elevation.ts
var elevation = (zValue) => {
  return `${config.elevation.level[zValue]} ${token({
    n: "elevationBase",
    o: config.elevation.opacity[zValue]
  })}`;
};
